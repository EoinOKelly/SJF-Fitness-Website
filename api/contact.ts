...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const service = cleanString(body.service, 200)
    ...
    if (
      ...
      return json('Please provide a valid name, email, phone number, service, and message.', 400)
    }
    ...
    try {
      const resendResponse = await fetch(RESEND_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          to: [to],
          reply_to: email,
          subject: `[SJF Fitness] ${service}`,
          text: `Name: ${name}\nEmail: ${email}\nPhone: ${phone}\nService: ${service}\n\n${message}`,
          html: `<h2>SJF Fitness website enquiry</h2><p><strong>Name:</strong> ${safeName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Phone:</strong> ${safePhone}</p><p><strong>Service:</strong> ${safeService}</p><hr><p>${safeMessage}</p>`,
        }),
      })
